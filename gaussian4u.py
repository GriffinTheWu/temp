...
def gaussian():
    ...
    for i in range(results.shape[0]):
        ...
        cov = np.asmatrix([[((y2 - y1) / 4.) **2, 0],[0, ((x2 - x1) / 4.) ** 2]],dtype=np.float64)
    ...
    for key in ans:
	    ...
	    score = i[2] * weight[key]
	    curr_box = [key,score,left,top,right,bottom]
	    if score > 0.5:
                im = draw_bounding_box(im,left,top,right,bottom)
	        testing.append(curr_box)
        misc.imsave('key.jpg',im)
    # save outputs to a txt file
    filename = '/home/griffin/tf-faster-rcnn/sku_test_object.txt'
